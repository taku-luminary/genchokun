...
export async function GET(
  ...
  try {
    ...
    // 2. 企業＋その企業が受けた全レビューを 1クエリで取得（reviewsReceived リレーション経由）。
    //    企業が存在すれば0件でも name が取れ、存在しなければ null → 404 にできる。
    ...
    const reviews = company.reviewsReceived;
    ...
    // 1件を表示用に整形する（overall は5項目の平均を小数1桁に丸める）
    const toItem = (r: (typeof reviews)[number]): CompanyReviewItem => {
      ...
      return {
        ...
        relatedPost: buildRelatedPost(r.match),
    ...
    // ロールごとに分ける
    const contractorReviews = reviews.filter((r) => r.targetRole === "contractor").map(toItem);
