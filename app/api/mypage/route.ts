...
export async function GET(): Promise<NextResponse<MypageApiResponse>> {
  ...
  // 応募した案件は一覧そのものを取得し、件数は配列の長さから数える。
  // 単純な matches.count({ where: { contractorUserId: user.id } }) では
  // 「自分の request に届いたマッチ(これも contractorUserId=自分)」を誤って数え、
  // 「request への自分の応募(こちらは salesUserId=自分)」を数え漏らすため。
  const [todoCount, projects, requests, appliedProjectMatches, appliedRequestMatches, myCompany] =
    await Promise.all([
      ...
      // レビュー待ち判定で「自社が既に書いたレビュー」を照合するため、自社の会社IDを取得
  ...
  // レビュー待ちになりうるのは active マッチだけ。4ソースから active マッチのIDを集める。
  ...
  // 応募カードに出す「相手企業」の会社IDを集める（合算★をまとめて取るため）
    ...
    return [
      {
        ...
        project: {
          ...
          // 相手（発注者）企業の合算★。0件なら null → カードで「（0件）」
  ...
  // 応募した request 一覧（構造は appliedProjects と同じ）
    ...
    return [
      {
        ...
        request: {
          ...
          summary: m.request.summary,
          paymentCycle: m.request.paymentCycle,
          rewardType: m.request.rewardType,
  ...
  // レビュー待ちの合計件数（4リストで reviewPending が true のもの）
  ...
  // 新しくマッチングの合計件数（4リストで newMatch が true のもの）
  const newMatchCount =
